#!/usr/bin/env node
/**
 * Audit verified Atlas Cloud model IDs before deploy.
 *
 * Usage:
 *   node scripts/verify-atlas-models.mjs           # list registry + check docs URLs
 *   node scripts/verify-atlas-models.mjs --json    # machine-readable output
 *
...
import {
  ...
  STUDIO_TIER_MODELS,
  listAllModelIds,
} from '../functions/lib/atlasModelRegistry.js';
...
async function main() {
  ...
  if (jsonMode) {
    ...
    console.log(`\nAtlas model registry — verified ${ATLAS_REGISTRY_VERIFIED_AT}\n`);
